 ...
 * {@link #parse(java.io.LineNumberReader, annotations.el.AScene)},
 * {@link #parseFile(String, annotations.el.AScene)}, and
 * {@link #parseString(String, annotations.el.AScene)}.
 * Each of these parses an index file into a {@link annotations.el.AScene}.
 * <p/>
 * <p/>
...
public final class IndexFileParser {
    ...
    private void expectChar(char c) throws IOException, ParseException {
        if (!matchChar(c)) {
            ...
            throw new ParseException("line " +st.lineno()+": expected '" + c + "', found " + found);
        }
    }
    ...
    private /*@ReadOnly*/ Object parseScalarAFV(ScalarAFT aft) throws IOException, ParseException {
        if (aft instanceof BasicAFT) {
            ...
            SceneAnnotation suba = parseAnnotationBody(d, ab2);
    ...
    private SceneAnnotation parseAnnotationBody(AnnotationDef d, AnnotationBuilder ab) throws IOException, ParseException {
        if (matchChar('(')) {
    ...
    private void parseAnnotations(AElement e)
        ...
        while (checkChar('@')) {
            ...
            if (ab == null) {
                ...
                SceneAnnotation a = parseAnnotationBody(d, ab);
                for (SceneAnnotation other : e.tlAnnotationsHere) {
                    if (a.def.name.equals(other.def.name)) {
                        throw new ParseException(
                                "Duplicate annotation of type " + a.def().name);
                    }
                }
                SceneAnnotation tla = a;
                if (!tla.def.equals(d)) {
    ...
    private ASTPath.ASTEntry parseASTEntry() throws IOException, ParseException {
        ...
        // TODO probably corresponds to Java 8
     ...
     * @return A new {@link annotations.io.ASTPath.ASTEntry}.
     ...
     * them into <code>scene</code>; see {@link #parse(java.io.LineNumberReader, annotations.el.AScene)}.
     */
    ...
    /**
     * Parse the given text into a {@link type.Type}.
     *
