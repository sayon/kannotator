...
private fun homogenizeScenes(fst: AScene, snd: AScene)
{
    homogenize(fst.packages, snd.packages)
    homogenize(fst.classes, snd.classes)

    for ((className, classRecord) in fst.classes)
        homogenize(classRecord, snd.classes.vivify(className))

    for ((className, classRecord) in snd.classes)
        homogenize(classRecord, fst.classes.vivify(className))
}

private fun homogenize(fst: AClass, snd: AClass)
{
...
private fun <K, V> homogenize (fst: VivifyingMap<K, V>?, snd: VivifyingMap<K, V>?) {
...
class ComparisonTest
{
    ...
    fun compareFiles(name: String) {
        ...
        //uncomment if you wish to write
